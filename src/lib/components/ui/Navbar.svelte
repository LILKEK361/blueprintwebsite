<script lang="ts">
    export let pages : string[] = []
    export let head : string
    const AllPages = {"OverAllList" : "/", "Trending" : "/Trendingpage"}

</script>
<div class="h-[10vh]">
    <div class="navbar bg-base-100">
        <div class="navbar-start">


            <div class="dropdown">
                <label tabindex="0" class="btn m-1">Pages</label>
                <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                    {#each pages as page}
                        <li><a href={AllPages[page]}>{page}</a></li>
                    {/each}
                </ul>
            </div>
        </div>
        <div class="navbar-center">
            <a class="btn btn-ghost normal-case text-xl">{head}</a>
        </div>
        <div class="navbar-end">
            <button class="btn btn-ghost btn-circle">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            </button>
            <div class="indicator">
                <div class="dropdown dropdown-end">
                    <label class="btn m-1" tabindex="1">ME</label>
                    <ul class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-[25vw]" tabindex="1">
                        <li>
                            <div class="card card-side bg-base-100 shadow-xl">

                                <div class="card-body">
                                    <h2 class="card-title">Yeap thats me: </h2>
                                    <div class="flex p-3">
                                        <div class="avatar">
                                            <div class="w-24 rounded-full">
                                                <img src="" width="auto" height="auto"/>
                                            </div>
                                            <p class="pl-10">Name: Nick Hillmann <br> Age: 18 <br> Job: None <br> Hobbys: Codding, Video Games</p>
                                        </div>
                                        <div class="card-actions justify-end">
                                            <a class="bg-secondary text-black rounded-full p-3" href="https://github.com/LILKEK361" target="_blank">GitHub</a>
                                        </div>
                                    </div>

                                </div>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>

        </div>
    </div></div>